...
export class AddCommissionComponent implements OnInit, OnDestroy {
  ...
  commissionGroup = new CommissionGroup().commissionGroup;
  ...
  ngOnInit() {
    this.initializeFormInputsAutoCompleteSubscriptions()
    this.validateDateControlOnValueChange();
    this.postCommissionToBackendWhenSubmit();
  }

  private initializeFormInputsAutoCompleteSubscriptions() {
  ...
  private validateDateControlOnValueChange() {
  ...
  private postCommissionToBackendWhenSubmit() {
